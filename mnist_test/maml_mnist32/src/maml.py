...
class MetaLearner(object):
    def __init__(self,
        ...
        num_in_channels = 1
        self.net = MnistNet(num_classes, self.loss_fn, num_in_channels)
        self.net.cuda()
        self.fast_net = InnerLoop(num_classes, self.loss_fn, self.num_inner_updates, self.inner_step_size, self.inner_batch_size, self.meta_batch_size, num_in_channels)
        self.fast_net.cuda()
    ...
    def test(self):
        num_in_channels = 1 
        test_net = MnistNet(self.num_classes, self.loss_fn, num_in_channels)
    ...
    def train(self, exp):
        ...
        for it in range(self.num_updates):
            ...
            for i in range(self.meta_batch_size):
                ...
                #print(task)
                self.fast_net.copy_weights(self.net)
                #print (self.fast_net.forward)
                #print(self.fast_net.forward(task))
                #mod = self.fast_net.forward(task)
                #metrics, g = mod.forward(task)
            ...
            np.save('../output/{}/meta_grad.npy'.format(exp), np.array(grads))
