...
class InnerLoop(MnistNet):
    ...
    def __init__(self, num_classes, loss_fn, num_updates, step_size, batch_size, meta_batch_size, num_in_channels=1):
    ...
    def forward_pass(self, in_, target, weights=None):
        ...
        #out = self.net_forward(torch.autograd.Variable(in_).cuda(async=True), weights)
    ...
    def forward(self, task):
        ...
        for i in range(self.num_updates):
            ...
            #in_, target = in_.to('cuda'), target.to('cuda')
            if i==0:
